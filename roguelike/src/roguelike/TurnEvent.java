...
public class TurnEvent {
    ...
    public static TurnEvent attackMissed(Actor initiator, Actor target, String message) {
        return new TurnEvent(initiator, target, ATTACK_MISSED).setMessage(message).setAnimation(new AttackMissedAnimation(target));
    }

    public static TurnEvent rangedAttack(Actor initiator, Actor target, Attack attack) {
        return new TurnEvent(initiator, target, RANGED_ATTACKED).setAnimation(new RangedAttackAnimation(initiator, target, "" + attack.getDamage()));
    }
