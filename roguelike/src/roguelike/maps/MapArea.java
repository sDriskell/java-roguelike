...
public class MapArea implements Serializable {
     ...
     *         map boundaries
     */
    public Coordinate getUpperLeftScreenTile(int screenCellsX, int screenCellsY,
            Coordinate center) {
    ...
    public boolean moveActor(Actor actor, Coordinate newPosition) {
        Coordinate pos = actor.getPosition();
        Tile tile = getTileAt(pos.x, pos.y);
        if (tile.getActor() != null && tile.moveActorTo(getTileAt(newPosition))) {
     ...
     *         the tile at the actor's position actually has no actor, which
