...
public abstract class MapBuilderBase implements Serializable {
    ...
    protected Rectangle getRandomRectangleInside(Rectangle containing) {
        int rectWidth = random.betweenWeighted(1, containing.width - 1, 5);
        int rectHeight = random.betweenWeighted(1, containing.height - 1, 5);

        int x = random.betweenWeighted((int) containing.getMinX(),
                (int) containing.getMaxX() - rectWidth, 5);
        int y = random.betweenWeighted((int) containing.getMinY(),
                (int) containing.getMaxY() - rectHeight, 5);
     ...
     * @return
     */
    protected Rectangle getSubRectangle(Rectangle original, int newX, int newY, double xPercent,
            double yPercent) {
        Rectangle newRect = new Rectangle(original);
        newRect.setLocation(newX, newY);

        newRect.setSize((int) Math.floor(original.width * xPercent),
                (int) Math.floor(original.height * yPercent));
        return newRect;
    }
