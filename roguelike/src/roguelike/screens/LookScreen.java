...
public class LookScreen extends CursorScreen {
    ...
    private class InformationPanel extends TextWindow {
        ...
        public void draw(TerminalBase terminal, MapArea map, int x, int y) {
            Rectangle bounds =
                    new Rectangle(lookPoint.x + 1, lookPoint.y + 1, size.width, size.height);
            ...
            TerminalBase lookTerm =
                    terminal.getWindow(bounds.x, bounds.y, bounds.width, bounds.height);
        ...
        private ArrayList<StringEx> getTextLines(MapArea map, int x, int y, boolean drawActor) {
            ...
            if (actor != null) {
                ...
                add(textList, "`White`" + equipped.name() + " ("
                        + equipped.defaultDamageType().name() + ")");
                add(textList, "");
                
                Statistics stats = actor.statistics();
                add(textList, String.format(
                        "`Bronze`Ref:`White`%3d `Bronze`Aim:`White`%3d `Bronze`Spd:`White`%3d",
                ...
                add(textList,
                        String.format(
                                " `Red`H:`White`%3d  `Bronze`MP:`White`%3d `Bronze`RP:`White`%3d ",
                                actor.health().getCurrent(), stats.baseMeleePool(0),
                                stats.baseRangedPool(0)));
            ...
            String[] itemDescriptions =
                    inventory.getGroupedItemListAsText(itemSize - BOTTOM_MARGIN);
