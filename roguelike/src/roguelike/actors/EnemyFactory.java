...
public class EnemyFactory {
  ...
  private EnemyFactory() {
    ...
    npcBuilders.put(EnemyType.WOLF, () -> NpcBuilder
        .withIdentifiers("wolf", 'w', SColor.LIGHT_GRAY, 1).withHealth(15).withVisionRadius(16)
        .withDescription("It looks at you with sinister eyes and snarls menacingly.").withSpeed(30)
        .withBehavior(a -> new SearchForPlayerBehavior(a)).withStats(5, 6, 6, 6, 4, 3)
        .equipItem(WeaponFactory.create(WeaponType.BITE), ItemSlot.RIGHT_HAND) // yes, this is funny
        .buildNpc());

    npcBuilders.put(EnemyType.FIRE_ANT,
        () -> NpcBuilder.withIdentifiers("fire ant", 'f', SColor.RED, 1).withHealth(8)
            .withDescription("A large ant, whose bite is said to cause immense pain.").withSpeed(15)
            .withBehavior(a -> new SearchForPlayerBehavior(a)).withStats(4, 3, 4, 4, 4, 4)
            .equipItem(WeaponFactory.create(WeaponType.MANDIBLES), ItemSlot.RIGHT_HAND).buildNpc());

    npcBuilders.put(EnemyType.BANDIT,
        () -> NpcBuilder.withIdentifiers("bandit", 'b', SColor.BRIGHT_PINK, 1).withHealth(25)
            .withDescription(
                "Waiting in ambush for their victims, these scoundrels prey upon the weak.")
            .withSpeed(10).withBehavior(a -> new SearchForPlayerBehavior(a))
            .withStats(6, 7, 7, 5, 6, 6).withInventory(inventoryBuilder.populateRandomInventory())
            .buildNpc());

    npcBuilders.put(EnemyType.SNAKE,
        () -> NpcBuilder.withIdentifiers("snake", 's', SColor.LIME, 1).withHealth(10)
            .withDescription("A small snake, slithering towards you.").withSpeed(20)
            .withBehavior(a -> new SearchForPlayerBehavior(a)).withStats(4, 5, 9, 8, 3, 3)
            .equipItem(WeaponFactory.create(WeaponType.BITE), ItemSlot.RIGHT_HAND).buildNpc());

    npcBuilders.put(EnemyType.ARCHER,
        () -> NpcBuilder.withIdentifiers("archer", 'a', SColor.LIGHT_MAROON, 1).withHealth(20)
            .withDescription("Probably a deserter from some king's army.").withSpeed(10)
            .withBehavior(a -> new MoveToRandomPointBehavior(a)).withStats(5, 4, 8, 8, 5, 4)
            ...
            .addItem(WeaponFactory.create(WeaponType.ARROW))
            .addItem(WeaponFactory.create(WeaponType.ARROW))
            .addItem(WeaponFactory.create(WeaponType.ARROW)).withAimingBonus(1).buildNpc());
